...
<html>
  ...
  <body>
    ...
    <div class="bg">
      ...
      <?php
        $firstNameErr = $lastNameErr = $genderErr = $dobErr =  $emailErr = $passwordErr = $conPasswordErr = "" ;
        ...
        if($conn1->connect_error) {
          ...
          $stmt1 = $conn1->prepare("select firstName, lastName, gender, dob, email, id, userName, password, conPassword from user where userName = ? ");
          $stmt1->bind_param("s", $p1);
          $p1 = $searchKey;
          $stmt1->execute();
          $res2 = $stmt1->get_result();
          $DBuser = $res2->fetch_assoc();
          ...
          $conPassword = $DBuser['conPassword'];
        ...
        if(isset($_POST['update']) && $password==$conPassword)
        {
          ...
          if($conn2->connect_error) {
            ...
            $stmt2 = mysqli_prepare($conn2, 'update user set firstName = ?, lastName = ?, gender = ?, dob = ?, email = ?, password = ?, conPassword = ? where userName = ?');
        ...
        if(isset($_POST['delete']))
        {
          ...
          if(mysqli_connect_error()) {
            ...
            $stmt3 = mysqli_prepare($conn3, 'delete from user where userName=?');
    ...
    function validate() {
      ...
      var date = document.forms["jsForm"]["dob"].value;
      ...
      var conpass = document.forms["jsForm"]["conPassword"].value;

      if(fname == "" || lname == "" || gender == "" || date == "" || email == "" || id == "" || uname == "" || password == "" || conpass == "") {

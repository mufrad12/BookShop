    ...
    <form name="jsForm" onsubmit="submitForm(event)">
      ...
      if($firstName != "" && $lastName != "" && $gender != "" && $dob != "" && $email != "" && $userName != "" && $password != "" && $conPassword != "" && $password == $conPassword)
      {
        ...
        if($conn->connect_error) {
          ...
          $stmt = $conn->prepare("insert into user (firstName, lastName, gender, dob, email, userName, password, conPassword) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            || email == "" || username == "" || password == "" || conPassword == "") 
          {
            ...
            xhttp.onreadystatechange = function() {
              if(this.readyState == 4 && this.status == 200) {
                document.getElementById("errorMsg").innerHTML = "<b>User Added Successfully</b>";
            ...
            xhttp.send("fname="+fname+"&lname="+lname+"&gender="+gender+"&dob="+dob+"&email="+email+"&uname="+username+"&password="+password+"&conPassword="+conPassword);
